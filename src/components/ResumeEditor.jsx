...
export default function ResumeEditor({ resume, setResume, openSections, setOpenSections, saveStatus }) {
  ...
  const resetAvatarFit = () => {
    updateGroup('basic', 'avatarFit', { x: 50, y: 50, scale: 1 });
  };
      ...
      <div className="space-y-4 p-5">
            ...
            {extraFields.map((field, index) => (
              <div key={field.id} className="grid grid-cols-[1fr_1.4fr_34px] gap-2">
                <TextInput
                  ...
                  value={field.label}
                  placeholder="意向城市"
                  onChange={(value) => updateExtraField(index, 'label', value)}
                />
                <TextInput
                  label="内容"
                  value={field.value}
        ...
        {!hiddenSections.includes('education') && (
          <EditorCard
            ...
            action={<DeleteSectionButton onClick={() => hideSection('education')} />}
          >
            <div className="grid gap-3 sm:grid-cols-2">
              {educationFields
                ...
                .map((field) => (
                  <div key={field.key} className="grid grid-cols-[1fr_34px] gap-2">
                    <TextInput
                      label={field.label}
                      value={resume.education[field.key]}
                      onChange={(value) => updateGroup('education', field.key, value)}
                    />
                    <button
                      type="button"
                      onClick={() => hideEducationField(field.key)}
                      className="mt-6 inline-flex h-9 w-8 items-center justify-center rounded-md text-slate-400 transition hover:bg-red-50 hover:text-red-600"
                      title={`删除${field.label}`}
                      aria-label={`删除${field.label}`}
                    >
              ...
              {!hiddenEducationFields.includes('courses') && (
                <div className="grid gap-2 sm:col-span-2 sm:grid-cols-[1fr_34px]">
                  ...
                  <button
                    type="button"
                    onClick={() => hideEducationField('courses')}
                    className="mt-6 inline-flex h-9 w-8 items-center justify-center rounded-md text-slate-400 transition hover:bg-red-50 hover:text-red-600"
                    title="删除主修课程"
                    aria-label="删除主修课程"
                  >
        ...
        {!hiddenSections.includes('projects') && (
          <EditorCard
            ...
            action={<DeleteSectionButton onClick={() => hideSection('projects')} />}
          >
            <ExperienceEditor
              titlePrefix="项目"
              nameLabel="项目名称"
              ...
              onChange={(value) => updateRoot('projects', value)}
            />
          </EditorCard>
        )}
        ...
        {!hiddenSections.includes('campus') && (
          <EditorCard
            ...
            action={<DeleteSectionButton onClick={() => hideSection('campus')} />}
          >
            <ExperienceEditor
              titlePrefix="经历"
              nameLabel="组织名称"
              roleLabel="角色"
              items={resume.campus}
              onChange={(value) => updateRoot('campus', value)}
            />
          </EditorCard>
        )}
        ...
        {!hiddenSections.includes('skills') && (
          <EditorCard
            ...
            action={<DeleteSectionButton onClick={() => hideSection('skills')} />}
          >
        ...
        {!hiddenSections.includes('honors') && (
          <EditorCard
            title="荣誉证书"
            open={openSections.honors}
            onToggle={() => toggleSection('honors')}
            action={<DeleteSectionButton onClick={() => hideSection('honors')} />}
          >
        ...
        {!hiddenSections.includes('summary') && (
          <EditorCard
            ...
            action={<DeleteSectionButton onClick={() => hideSection('summary')} />}
          >
            <TextArea
              label="自我评价"
              rows={6}
